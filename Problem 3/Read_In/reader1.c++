...
#define max_y 607     	//y grid points of data file                
...
int main()
{
  ...
  ifstream fp("potentialarrayEx1.txt");
  ...
  ifstream fp2("maskarrayEx1.txt");		//read in masked array
  for (int i=0; i<max_x; i++){			
    for(int j=0; j<max_y; j++){
  ...
  for(iter=0; iter<10000; iter++)               /* iterations */
   ...
   cout << "Data stored in reader1.dat"<< endl;
   myfile.close();
